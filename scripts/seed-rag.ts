...
import { EMBEDDING_DIMENSION } from "../src/lib/rag/types.ts";
...
async function main(): Promise<void> {
  const { OPENAI_API_KEY } = getEmbeddingEnv();
  ...
  console.log(`[seed-rag] 向量化 ${SEED_KNOWLEDGE.length} 条知识…`);
  const embedding = await openai.embeddings.create({
    model: embeddingModel,
    input: SEED_KNOWLEDGE.map((k) => k.content),
  });
