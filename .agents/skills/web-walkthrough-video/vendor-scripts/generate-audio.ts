 ...
 *   edge                 — Microsoft Edge TTS (free; good Chinese voices via edge-tts CLI)
 *
...
function resolveVoices(lang: string | undefined) {
  ...
  return {
    ...
    edge:       (L && process.env[`EDGE_TTS_VOICE_${L}`])      ?? process.env.EDGE_TTS_VOICE     ?? 'zh-CN-XiaoxiaoNeural',
  };
}
...
async function generateEdge(text: string, outputPath: string, voiceName: string): Promise<void> {
  ...
  if (result.status !== 0) {
    throw new Error(`edge-tts failed: ${result.stderr || result.stdout || result.error}`);
  }
}
...
async function main(): Promise<void> {
  ...
  if (PROVIDER === 'edge') {
    ...
    const edgeBin =
      process.env.EDGE_TTS_BIN ??
      `${process.env.HOME}/Library/Python/3.10/bin/edge-tts`;
    const check = spawnSync(edgeBin, ['--help'], { encoding: 'utf-8' });
    if (check.error || (check.status !== 0 && check.status !== null && !`${check.stdout}${check.stderr}`.includes('edge-tts'))) {
      // --help may exit 0; only fail if binary missing
      if (check.error) {
        console.error(`edge-tts not found at ${edgeBin}. Run: python3 -m pip install --user edge-tts`);
        process.exit(1);
      }
    }
  }
  ...
  const label = PROVIDER === 'openai'
    ...
    : PROVIDER === 'edge'
    ? `Edge TTS / ${voices.edge}${lang ? ` [${lang}]` : ''}`
  ...
  for (const step of flow.steps) {
    ...
    if (PROVIDER === 'openai') {
      ...
      await generateEdge(step.narration, audioPath, voices.edge);
    } else {
